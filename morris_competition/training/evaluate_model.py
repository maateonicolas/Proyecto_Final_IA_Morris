...
#
# Evalúa que el checkpoint entrenado pueda cargarse y producir:
# - una política sobre acciones legales
# - un valor estimado del estado
...
CHECKPOINT_PATH = ROOT / "training" / "checkpoints" / "morris_net.pt"


def load_checkpoint(path=CHECKPOINT_PATH, device="cpu"):
    if not path.exists():
        raise FileNotFoundError(
            f"No se encontró checkpoint: {path}\n"
            f"Primero ejecuta: python training/train.py"
        )
...
def evaluate_initial_state():
    ...
    model, checkpoint = load_checkpoint(device=device)
    ...
    print("=" * 70)
    print("EVALUATE MODEL — MorrisNet")
    print("=" * 70)
